...
public abstract class BaseCacheManager implements CacheManager {
     ...
     * 是否允许空值
     */
    protected boolean allowNullValue = true;
    ...
    @Override
    public Cache getCache(String cacheName, LayerCacheSetting layerCacheSetting) {
        ...
        synchronized (this.cacheContainer) {
            // 新建一个Cache对象
            cache = getMissingCache(cacheName, layerCacheSetting);
            if (cache != null) {
                // 装饰Cache对象
