...
import com.y3tu.tools.lowcode.common.entity.constant.DataStatusEnum;
...
import com.y3tu.tools.lowcode.common.repository.DictRepository;
...
public class DictServiceImpl extends BaseServiceImpl<DictRepository, Dict> implements DictService {
    ...
    public List<DictData> getDictData(String code, Object... targetValue) {
        ...
        if (dict == null) {
            throw new LowCodeException(String.format("字典编码[s]不存在", code));
        }
        ...
        if (Dict.DictType.COMMON.getValue().equals(dict.getType())) {
            ...
            if (dictSql == null) {
                throw new LowCodeException(String.format("字典[%s]不存在", code));
            } else if (DataStatusEnum.DISABLE.getValue() == dictSql.getStatus()) {
                throw new LowCodeException(String.format("字典[%s]被禁用", code));
            }
            ...
            try {
                ...
                throw new LowCodeException(String.format("字典[%s]解析异常", code), e);
            }
